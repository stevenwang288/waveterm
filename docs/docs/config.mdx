...
WaveCN's configuration files are located at `~/.config/waveterm-zhcn/` (dev builds use `~/.config/waveterm-zhcn-dev/`).

The main configuration file is `settings.json` (`~/.config/waveterm-zhcn/settings.json`).
...
| term:wheellinescroll                 | bool     | when enabled, mouse wheels scroll the terminal by 1 line per notch (touchpads remain smooth). defaults to true                                                                                                                                              |
| term:scrollback                      | int      | size of terminal scrollback buffer, max is 10000                                                                                                                                                                                                              |
| term:activitywindowms                | int      | used to infer a terminal's "running" state when shell integration is unavailable. defaults to 60000 (60s)                                                                                                                                                   |
...
| tab:sidepanelbuttonorder             | string[] | order of the top bar quick buttons that open the left side panel. defaults to `["ai","favorites","servers","layouts","git"]`                                                                                                                                 |
