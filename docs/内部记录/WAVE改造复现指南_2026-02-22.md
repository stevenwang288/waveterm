...
- 分支：`master`
- 目标提交（commit）：以 `master` 最新为准（`git rev-parse HEAD`）
- 版本号：`0.14.0-7`
- Windows 安装包（本机交付落点）：`D:\DESK\WAVE-win32-x64-0.14.0-7.exe`
...
3. 安装依赖并打包：
   ...
   - `task package`
4. 产物：
   - `make\0.14.0-7\WAVE-win32-x64-0.14.0-7.exe`
5. 放到桌面（你机器约定）：
   - 复制到 `D:\DESK\WAVE-win32-x64-0.14.0-7.exe`
...
4. 验证版本号：
   - 检查 `package.json` 的 `version` 为 `0.14.0-7`
