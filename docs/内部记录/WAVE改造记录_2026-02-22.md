...
交付安装包：`D:\DESK\WAVE-win32-x64-0.14.0-7.exe`
...
- Windows 安装包版本采用 `0.14.0-<patch>` 递增：
  - `0.14.0-1` → `0.14.0-2` → `0.14.0-3` → `0.14.0-4` → `0.14.0-5` → `0.14.0-6` → `0.14.0-7`
- 目的：
  - 每次交付一个“你能安装验证”的明确版本；
  - 避免多个安装包名字/版本混在一起，难以回滚/对比。

### 1.3 交付落点（你固定工作流）
- 你的 Windows 桌面目录固定为：`D:\DESK`。
...
- 当检测到 Codex UI 特征时：
  - **优先**提取以 `•`/`●` 开头的最后一段 assistant 回复（绿底常见形态）；
  - 若没有 bullet 回复，则仅在**严格模式**（要求回复后出现下一条 prompt）下，允许提取“纯文本最终回复”（同样会经过噪声过滤）。
- 对提取到的段落进行噪声过滤：
...
**补强：避免“读不完/中途截断/朗读失败”**
- 之前语音输入有长度上限（超过会被截断），导致“绿底正式回复”较长时播报不完整。
- 现在改为 **分段 chunk**：
  - chunk 切分：`frontend/app/aipanel/aispeech.ts`（`chunkSpeechInput()`）
  - API 播放串联：`frontend/app/aipanel/speechruntime.ts`（分段请求/分段播放，播完才结束）
...
**交付版本**
- 最新：`0.14.0-7`（`D:\DESK\WAVE-win32-x64-0.14.0-7.exe`）
...
#### 2.2.1 Windows：AI CLI（Codex）绿底/背景色不显示
**现象**
- 旧版终端里 Codex TUI 的“assistant 正式回复段（绿底）”可见；
- 新版里背景色丢失（看起来像“没有绿底”），影响你判断“哪些内容该被朗读”。

**处理策略**
- Windows 默认禁用终端 WebGL 渲染（让 ANSI 背景色渲染稳定可见）。
- 仍可通过配置显式开启/关闭：
  - `term:disablewebgl = true`：禁用 WebGL
  - `term:disablewebgl = false`：启用 WebGL
...
- 语音播报：长回复改为分段播报（避免“读不完/中途截断/朗读失败”）
- 终端标题 cwd 跳回/不匹配：收到 OSC7 cwd 后停止“命令推断 cwd”，避免标题路径乱跳
- Windows：释放 `Alt+W`（不再触发关闭）
- Windows：终端默认禁用 WebGL（修复 Codex TUI 绿底/背景色不显示）
- 推送到在线仓库：`origin/master`（以 `0.14.0-7` 为准）
- 交付安装包：`D:\DESK\WAVE-win32-x64-0.14.0-7.exe`
...
- 状态：已修复（`0.14.0-6`）。当 shell 已发出 OSC7 cwd 时，停止基于命令行的“推断 cwd”，避免标题路径跳回。
- 代码：`frontend/app/view/term/termwrap.ts`
...
### 5.3 Alt+W 关闭窗口
- 你要求：释放 `Alt+W`，不能关闭窗口。
- 状态：已修复（`0.14.0-6`）。Windows 下不再绑定 `Cmd+w`（映射为 `Alt+W`）到关闭行为。
- 代码：`frontend/app/store/keymodel.ts`
...
- 状态：
  - 推送远端：已完成（`origin/master` 已更新到 `0.14.0-7`，并已产出安装包到 `D:\DESK`）。
  - 上游同步：仍待决策（当前与 `upstream/main` 历史疑似不相干，无法直接 merge；需要选择“重建到上游再移植改造”或“继续快照式拣选同步”）。
