    ...
    return (
        <div className="flex items-center gap-4 py-2 border-b border-border">
            <Tooltip
                content={
                    !isMapped
                        ? t("builder.secretsTab.status.notMapped")
                        : isValid
                          ? t("builder.secretsTab.status.validMapping")
                          : t("builder.secretsTab.status.invalidBinding")
                }
            >
            ...
            <div className="flex items-center gap-2">
                ...
                {!isMapped && !hasMatchingSecret && (
                    <button
                        ...
                        className="px-3 py-1 text-sm font-medium rounded bg-accent/80 text-primary hover:bg-accent transition-colors cursor-pointer whitespace-nowrap"
                    >
                        {t("builder.actions.setAndMapDefault")}
                    </button>
                )}
            </div>
        </div>
...
const SetSecretDialog = memo(({ secretName, onSetAndMap }: SetSecretDialogProps) => {
        ...
        return (
            ...
            >
                <div className="flex flex-col gap-4 mb-4">
                    <h2 className="text-xl font-semibold">{t("builder.secretsTab.setSecret.errorTitle")}</h2>
    ...
    return (
        <Modal
            ...
            onClose={handleClose}
            okLabel={t("builder.actions.setAndMap")}
            cancelLabel={t("common.cancel")}
            okDisabled={!secretValue.trim() || isSubmitting}
        >
            <div className="flex flex-col gap-4 mb-4">
                <h2 className="text-xl font-semibold">{t("builder.secretsTab.setSecret.title")}</h2>
                <div className="flex flex-col gap-2">
                    <div className="text-sm font-medium mb-1">
                        {t("builder.secretsTab.setSecret.secretNameLabel")}{" "}
                        <span className="text-accent">{secretName}</span>
                    </div>
                    <textarea
                        value={secretValue}
                        onChange={(e) => setSecretValue(e.target.value)}
                        placeholder={t("builder.secretsTab.setSecret.valuePlaceholder")}
                    ...
                    <div className="text-xs text-secondary">
                        {t("builder.secretsTab.setSecret.storageHint")}
                    </div>
                </div>
            </div>
        </Modal>
...
const BuilderSecretTab = memo(() => {
        ...
        return (
            <div className="w-full h-full flex items-center justify-center">
                <div className="text-secondary text-center">
                    {t("builder.secretsTab.manifestNotAvailable")}
                </div>
            </div>
        );
    }
    ...
    const handleMapDefault = async (secretName: string) => {
        ...
        try {
            ...
            globalStore.set(model.errorAtom, t("builder.secretsTab.errors.saveBindingsFailed", { errorMessage }));
        }
    };
            ...
            <div className="flex-1 overflow-auto">
                {sortedSecretEntries.length === 0 ? (
                    <div className="text-secondary text-center py-8">
                        {t("builder.secretsTab.emptyState")}
                    </div>
                ) : (
