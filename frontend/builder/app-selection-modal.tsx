...
function CreateNewWaveApp({ onCreateApp }: { onCreateApp: (appName: string) => Promise<void> }) {
    ...
    const validateAppName = (name: string) => {
        ...
        if (name.length > MaxAppNameLength) {
            setInputError(t("builder.appSelection.validation.maxLength", { max: MaxAppNameLength }));
            return false;
        }
        if (!AppNameRegex.test(name)) {
            setInputError(t("builder.appSelection.validation.allowedChars"));
            return false;
        }
        setInputError("");
        return true;
    ...
    return (
        <div className="min-h-[80px]">
            <h3 className="text-base font-medium mb-1 text-muted-foreground">{t("builder.appSelection.createNew")}</h3>
            <div className="relative">
                <div className="flex w-full">
                    <input
                        ...
                        placeholder={t("builder.appSelection.namePlaceholder")}
...
export function AppSelectionModal() {
    ...
    const loadApps = async () => {
        try {
            ...
            setError(t("builder.appSelection.failedToLoadApps"));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSelectApp = async (appId: string) => {
        ...
        if (appId.startsWith("local/")) {
            try {
                ...
                setError(t("builder.appSelection.failedToCreateDraftFromLocal", { appId, errorMessage }));
                return;
            }
        }
        ...
        document.title = t("builder.windowTitle", { appId: appIdToUse });
    ...
    const handleCreateNew = async (appName: string) => {
        ...
        document.title = t("builder.windowTitle", { appId: draftAppId });
    ...
    const getAppDisplayName = (appId: string) => {
        ...
        if (parts.length === 2) {
            ...
            return isDraft ? `${parts[1]} ${t("builder.appSelection.draftSuffix")}` : parts[1];
        }
        return appId;
    };
    ...
    if (loading) {
        return (
            <FlexiModal className="min-w-[600px] w-[600px]">
                <div className="text-center py-8">{t("builder.appSelection.loadingApps")}</div>
            </FlexiModal>
        );
    }
    ...
    return (
        <FlexiModal className="min-w-[600px] w-[600px] max-h-[90vh] overflow-y-auto">
            <div className="w-full px-2 pt-0 pb-4">
                <h2 className="text-2xl mb-2">{t("builder.appSelection.selectToEdit")}</h2>
                ...
                {apps.length > 0 && (
                    <div className="mb-2">
                        <h3 className="text-base font-medium mb-1 text-muted-foreground">{t("builder.appSelection.existingWaveApps")}</h3>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[11px] text-muted mt-0.5">
                                                {t("builder.appSelection.lastUpdated", {
                                                    relativeTime: formatRelativeTime(appInfo.modtime),
                                                })}
                                            </span>
                                        </div>
                                    </div>
                ...
                {apps.length > 0 && (
                    <div className="flex items-center gap-4 my-2">
                        ...
                        <span className="text-muted-foreground text-sm">{t("builder.appSelection.or")}</span>
                        <div className="flex-1 border-t border-border"></div>
                    </div>
                )}
