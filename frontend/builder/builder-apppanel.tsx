...
const PublishAppModal = memo(({ appName }: { appName: string }) => {
    ...
    const handlePublish = async () => {
        if (!builderAppId) {
            setErrorMessage(t("builder.publish.noBuilderAppId"));
            setState("error");
            return;
        }
    ...
    if (state === "success") {
        return (
            <Modal className="p-4" onOk={handleClose} onClose={handleClose} okLabel={t("common.ok")} cancelLabel="">
                <div className="flex flex-col gap-4 mb-4">
                    <h2 className="text-xl font-semibold flex items-center gap-2">
                        <i className="fa fa-check-circle text-success" />
                        {t("builder.publish.successTitle")}
                    </h2>
                    <div className="flex flex-col gap-3">
                        <p className="text-primary">
                            {t("builder.publish.publishedTo")} <span className="font-mono">{publishedAppId}</span>
                        </p>
                    </div>
                </div>
            </Modal>
    ...
    return (
        <Modal
            ...
            onClose={handleClose}
            okLabel={t("builder.actions.publish")}
            cancelLabel={t("common.cancel")}
        >
            <div className="flex flex-col gap-4 mb-4">
                <h2 className="text-xl font-semibold">{t("builder.publish.confirmTitle")}</h2>
                <div className="flex flex-col gap-3">
                    <p className="text-primary">
                        {t("builder.publish.publishTo")} <span className="font-mono">{`local/${appName}`}</span>
                    </p>
                    <p className="text-warning">
                        <i className="fa fa-triangle-exclamation mr-2" />
                        {t("builder.publish.overwriteWarning")}
                    </p>
                </div>
            </div>
        </Modal>
    ...
    const handleKebabClick = useCallback(
        (e: React.MouseEvent) => {
            const menu: ContextMenuItem[] = [
                {
                    label: t("builder.actions.publishApp"),
                ...
                {
                    label: t("builder.actions.switchApp"),
            ...
            <div className="shrink-0 border-b border-border">
                <div className="flex items-center justify-between">
                    <div className="flex">
                        ...
                        />
                        <TabButton
                            label={t("builder.tabs.configData")}
                            tabType="configdata"
