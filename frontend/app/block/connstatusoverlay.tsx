...
export const ConnStatusOverlay = React.memo(
    ({
        ...
        let statusText = t("connStatus.disconnected", { connName });
        let showReconnect = true;
        if (connStatus.status == "connecting") {
            statusText = t("connStatus.connecting", { connName });
            showReconnect = false;
        }
        ...
        const handleCopy = React.useCallback(
            async (e: React.MouseEvent) => {
                const errTexts = [];
                if (showError) {
                    errTexts.push(t("connStatus.errorLine", { error: connStatus.error }));
                }
                if (showWshError) {
                    errTexts.push(t("connStatus.wshErrorLine", { error: connStatus.wsherror }));
                }
                        ...
                        <div className="connstatus-status ellipsis">
                            ...
                            {(showError || showWshError) && (
                                ...
                                >
                                    ...
                                    {showError ? <div>{t("connStatus.errorLine", { error: connStatus.error })}</div> : null}
                                    {showWshError ? (
                                        <div>{t("connStatus.wshErrorLine", { error: connStatus.wsherror })}</div>
                                    ) : null}
                                </OverlayScrollbarsComponent>
                            )}
                            {showWshError && (
                                <Button className={reconClassName} onClick={handleDisableWsh}>
                                    {t("connStatus.disableWshAlways")}
                                </Button>
                            )}
                        </div>
                    </div>
