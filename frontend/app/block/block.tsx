...
import { PveConsoleViewModel } from "@/app/view/pve/pveconsole";
import { PveScreenwallViewModel } from "@/app/view/pve/pvescreenwall";
...
BlockRegistry.set("pvescreenwall", PveScreenwallViewModel);
BlockRegistry.set("pveconsole", PveConsoleViewModel);
...
function getViewElem(
    ...
    if (isBlank(blockView)) {
        return <CenteredDiv>{i18next.t("block.noView")}</CenteredDiv>;
    }
    if (viewModel.viewComponent == null) {
        return <CenteredDiv>{i18next.t("block.noViewComponent")}</CenteredDiv>;
    }
...
const BlockSubBlock = memo(({ nodeModel, viewModel }: FullSubBlockProps) => {
    ...
    return (
        <div key="content" className={clsx("block-content", { "block-no-padding": noPadding })} ref={contentRef}>
            <ErrorBoundary>
                <Suspense fallback={<CenteredDiv>{t("common.loading")}</CenteredDiv>}>{viewElem}</Suspense>
            </ErrorBoundary>
        </div>
    );
});
