...
function createDefaultPlotMeta() {
    const meta = {
        cpu: defaultCpuMeta(i18next.t("sysinfo.cpuPercent")),
        "mem:total": defaultMemMeta(i18next.t("sysinfo.memoryTotal"), "mem:total"),
        "mem:used": defaultMemMeta(i18next.t("sysinfo.memoryUsed"), "mem:total"),
        "mem:free": defaultMemMeta(i18next.t("sysinfo.memoryFree"), "mem:total"),
        "mem:available": defaultMemMeta(i18next.t("sysinfo.memoryAvailable"), "mem:total"),
    };
    for (let i = 0; i < 32; i++) {
        meta[`cpu:${i}`] = defaultCpuMeta(`${i18next.t("sysinfo.core")} ${i}`);
    }
    return meta;
}
...
class SysinfoViewModel implements ViewModel {
    ...
    constructor(blockId: string, nodeModel: BlockNodeModel, tabModel: TabModel) {
        ...
        this.plotMetaAtom = jotai.atom(new Map(Object.entries(createDefaultPlotMeta())));
    ...
    getSettingsMenuItems(): ContextMenuItem[] {
        ...
        fullMenu.push({
            label: i18next.t("sysinfo.plotType"),
            submenu: submenu,
        });
