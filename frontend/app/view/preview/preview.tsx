...
import { ExplorerDirectoryPreview } from "./explorer-directory";
...
const SpecializedViewMap: { [view: string]: ({ model }: SpecializedViewProps) => React.JSX.Element } = {
    ...
    "explorer-directory": ExplorerDirectoryPreview,
};
...
const SpecializedView = memo(({ parentRef, model }: SpecializedViewProps) => {
    ...
    if (!SpecializedViewComponent) {
        return (
            <CenteredDiv>
                {i18next.t("preview.invalidSpecializedView", { view: specializedView.specializedView })}
            </CenteredDiv>
        );
    }
    ...
    return (
        <>
            ...
            <BlockHeaderSuggestionControl
                ...
                placeholderText={i18next.t("preview.openFilePlaceholder")}
            />
        </>
    );
}
