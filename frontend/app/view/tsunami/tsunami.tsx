...
class TsunamiViewModel extends WebViewModel {
    ...
    constructor(blockId: string, nodeModel: BlockNodeModel, tabModel: TabModel) {
        ...
        this.viewName = jotai.atom((get) => {
            const meta = get(this.appMeta);
            return meta?.title || i18next.t("term.waveApp");
        });
    ...
    getSettingsMenuItems(): ContextMenuItem[] {
        ...
        // Avoid substring matching on labels because labels are localized.
        const hiddenLabels = new Set<string>([
            i18next.t("webview.setBlockHomepage"),
            i18next.t("webview.setDefaultHomepage"),
            i18next.t("webview.hideNavigation"),
            i18next.t("webview.showNavigation"),
        ]);
        const filteredItems = items.filter((item) => (item.label ? !hiddenLabels.has(item.label) : true));
        ...
        const tsunamiItems: ContextMenuItem[] = [
            {
                label: i18next.t("waveapp.stop"),
                click: () => this.destroyController(),
            },
            {
                label: i18next.t("waveapp.restart"),
                click: () => this.restartController(),
            },
            {
                label: i18next.t("waveapp.restartForceRebuild"),
        ...
        if (showRemixOption) {
            tsunamiItems.push(
                {
                    label: i18next.t("waveapp.remixInBuilder"),
...
const TsunamiView = memo((props: ViewComponentProps<TsunamiViewModel>) => {
    ...
    const errors = [];
    if (!appPath && !appId) {
        errors.push(t("tsunami.errors.missingAppPathOrId"));
    }
    if (controller !== "tsunami") {
        errors.push(t("tsunami.errors.invalidController"));
    }
    ...
    return (
        <div className="w-full h-full flex flex-col items-center justify-center gap-4">
            ...
            {isRestarting && <div className="text-sm text-success-color">{t("waveapp.starting")}</div>}
        </div>
    );
});
