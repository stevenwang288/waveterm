...
import {
    buildConnectionName,
    compareServerEntries,
    doesConnectionMatchMachine,
    getConnectionHostCandidates,
    getConnectionVmid,
    getManagedConnectionHost,
    normalizeHostForMatch,
    parseConnectionName,
    reconcilePveConnections,
...
const ServersPanel = memo(() => {
    ...
    const remoteEntries = useMemo<ServerEntry[]>(() => {
        return uniqNonBlank(connections)
    ...
    const managedEntries = useMemo(() => {
        ...
        return [...pveEntries, ...unmatchedManaged].sort(compareServerEntries);
    ...
    const startPveRefresh = useCallback(
            ...
            fireAndForget(async () => {
                try {
                    ...
                    try {
                        ...
                        // ignore list reload failures after a refresh error
                    }
                    const now = Date.now();
                    pushNotification({
    ...
    const showPanelContextMenu = useCallback(
        ...
        [openAddModal, openConnectionsEditor, refreshServers, t]
    );
        ...
        <div className="flex flex-col h-full bg-zinc-950 border-r border-zinc-800 overflow-hidden" onContextMenu={showPanelContextMenu}>
            <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-800 flex-shrink-0">
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        <i className={pveRefreshing ? "fa fa-spinner fa-spin" : "fa fa-rotate-right"} />
                    </div>
                </div>
            </div>
