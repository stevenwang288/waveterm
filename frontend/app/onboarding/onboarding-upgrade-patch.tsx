...
const UpgradeOnboardingPatch = () => {
    ...
    const prevVersion = hasPrev ? versions[currentIndex - 1].version : null;
    const nextVersion = hasNext ? versions[currentIndex + 1].version : null;
                ...
                <div className="flex flex-col h-full">
                    <header className="flex flex-col gap-2 border-b-0 p-0 mt-1 mb-6 w-full unselectable flex-shrink-0">
                        ...
                        <div className="text-center text-[25px] font-normal text-foreground">
                            {t("onboarding.upgradePatch.title", { version: currentVersion.version })}
                        </div>
                    </header>
                            ...
                            <div className="flex-1 flex justify-start">
                                {hasPrev && (
                                    <div className="text-sm text-secondary">
                                        ...
                                        >
                                            &lt; {t("onboarding.upgradePatch.prevWithVersion", { version: prevVersion })}
                                        </button>
                                    </div>
                                )}
                            </div>
                                ...
                                {hasNext && (
                                    <div className="text-sm text-secondary">
                                        <button
                                            ...
                                            className="cursor-pointer hover:text-foreground transition-colors"
                                        >
                                            {t("onboarding.upgradePatch.nextWithVersion", { version: nextVersion })} &gt;
                                        </button>
                                    </div>
                                )}
                            </div>
