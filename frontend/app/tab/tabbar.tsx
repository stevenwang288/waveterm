...
const ConfigErrorMessage = () => {
    ...
    if (fullConfig?.configerrors == null || fullConfig?.configerrors.length == 0) {
        return (
            <div className="max-w-[500px] p-5">
                <h3 className="font-bold text-base mb-2.5">{t("tab.configClean")}</h3>
                <p>{t("tab.configCleanDesc")}</p>
            </div>
        );
    }
    if (fullConfig?.configerrors.length == 1) {
        ...
        return (
            <div className="max-w-[500px] p-5">
                <h3 className="font-bold text-base mb-2.5">{t("tab.configError")}</h3>
                <div>
...
const TabBar = memo(({ workspace }: TabBarProps) => {
    ...
    const addtabButtonDecl: IconButtonDecl = {
        ...
        icon: "plus",
        click: handleAddTab,
        title: useTranslation().t("tab.addTab"),
    };
    return (
